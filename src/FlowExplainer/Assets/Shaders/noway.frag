...
void main()
{
    ...
    vec4 col = mix(texture(mainTex2, vec2(vertexColor.y, vertexColor.z)), texture(mainTex, vec2(vertexColor.y, vertexColor.z)), alpha);
    //col = vec4(1,0,1,1);
    col.a = 1;
    vec3 lightCol = vec3(0,0,0);

    vec3 lightDir = normalize(vec3(-.9, -.9, -1.1));
   // vec3 lightDir = normalize(vec3(-.8, -.4, -.5));
   ...
   // color = vec4(vertexColor.y,vertexColor.z,0,1);
