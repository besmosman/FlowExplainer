...
public class DensityPathStructures2 : WorldService, IAxisTitle
{
    ...
    public struct Particle
    {
        ...
        public bool Marked;
        public double TimeMarked;
        public double IntegrationTimeLeft;
    }
    ...
    public override void Draw(View view)
    {
        ...
        var Tconv = GetRequiredWorldService<DataService>().LoadedDataset.GetVectorField<Vec3,double>("Convective Temperature");
        ...
        var rect = dat.VectorField.Domain.RectBoundary;
        var seedRect = new Rect<Vec3>(new Vec3(rect.Min.XY, dat.SimulationTime +3), new Vec3(rect.Max.XY, dat.SimulationTime + 5f));

        var targetTime = dat.SimulationTime;
        ...
        Parallel.For(0, Particles.Length, i =>
        {
            ...
            int c = 0;
            while (p.IntegrationTimeLeft > 0.0001 && c < 1)
            {
                ...
                c++;
                p.IntegrationTimeLeft -= double.Abs(deltaFicitious);

                
            }
            
            if (dt != 0 && Utils.Random(0, 1) > 1f - reseedRate || p.TimeMarked > 1)
            {
                var random = Utils.Random(seedRect);
                p.Position = random.XY;
                p.LastPosition = p.Position;
                p.PhysicalTime = random.Last;
                p.Marked = false;
                p.IntegrationTimeLeft = 30;
                p.LastPhysicalTime = p.PhysicalTime;
                p.VisualTimeAlive = 0;
                p.TimeMarked = 0;
            }
        ...
        Parallel.For(0, Particles.Length, c =>
                //for (int c = 0; c < Particles.Length; c++)
            {
                ...
                if (!(p.PhysicalTime < dat.SimulationTime) || (p.TimeMarked > 1.1f))
                {
                    return;
                }

                p.Marked = true;
                p.TimeMarked += FlowExplainer.DeltaTime;
