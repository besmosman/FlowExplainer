    ...
    public static RegularGridVectorField<Vec3, Vec3i, double> Convert(HeatSimulation simulation)
    {
        ...
        foreach (var state in simulation.States)
        {
            for (int i = 0; i < state.ParticleX.Length; i++)
            {
                ...
                var t = state.Time;
                var voxelCoord = heatField.ToVoxelCoord(new Vec3(x, y, t)).RoundInt();
...
public class FDTest : WorldService
{
    ...
    public override void Draw(RenderTexture rendertarget, View view)
    {
        ...
        for (int x = 0; x < Temprature.GridSize.X; x++)
        {
            ...
            Temprature.AtCoords(new Vec2i(x, Temprature.GridSize.Y - 1)) = 0;
        }
        ...
        {
            FiniteDifferences.Test(vel, Temprature, dat.SimulationTime, dat.MultipliedDeltaTime/steps);
        }
