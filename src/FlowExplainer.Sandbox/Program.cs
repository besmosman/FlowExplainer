...
while (true)
{
    ...
    var fluxNormalized = new ArbitraryField<Vec2, Vec2>(u.Domain,
        (x) => u.Evaluate(x).NormalizedSafe());
    IIntegrator<Vec2, Vec2>.Rk4Steady.Integrate(fluxNormalized, Vec2.Zero, .1);
}
*/
...
while (true)
{
    ...
    var h = 0.00001;
    var d = Vec2.Zero;
    for (int i = 0; i < xx.ElementCount; i++)
    {
        var leftCoords = xx;
        var rightCoords = xx;
        leftCoords[i] -= h;
